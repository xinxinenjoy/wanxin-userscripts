 ...
 *
 * 当前功能：
 * - SOA订单页优先读商机编号，老订单缺失时用单位代码，并识别订单编号。
 * - 日期区间可自定义；默认结束今天、开始取“报价确认”日期；超 1080 天自动拆多段查询并汇总。
 * - 支持多种日期格式输入并自动标准化；支持网络错误 / 超时 / 429 / 5xx 重试及 Excel 固定命名下载。
 * - 优先复用 Fly 有效 Token；缺失或失效时才打开一次 Fly 页面刷新授权。
 * - 查询 totalAmount / creditAmount，并与体检数据的到检总额、挂账金额做差异比对。
 * - 查询成功后才显示导出区；多区间分别显示日期并独立导出。
 * - 面板支持拖动并保存位置；后续其他 Fly 同类型报表导出统一在本脚本扩展。
 */
