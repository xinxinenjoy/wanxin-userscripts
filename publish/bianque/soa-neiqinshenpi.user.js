 ...
 * 更新记录
 *
 * v2.2.0 - 2026-9-16（按红领巾反馈改）
 * - 平时**不检测状态**：去掉启动时的 refreshState()，只有展开面板那一刻才读；
 *   开关文案随之固定为「订单特殊审批」（不读就不显示状态，免得变相天天请求）。
 * - **地区跟着页面走**：地区码改为严格从 localStorage.REGION_CODE_KEY 取，
 *   去掉原来 `|| 'XX'` 的静默兜底 —— 读不到就抛错中止，绝不用默认值写生产。
 *   面板新增「当前地区」一行（显示 XX（新乡地区）），让"跟页面走"看得见。
 * - **无操作权限提示**：展开面板先查 localStorage.userpermission 里有没有 CONFIG_MGMT
 *   （精确匹配，防 CONFIG_MGMT_ENTRANCE 被前缀误命中）；没有就直接显示「无操作权限」、
 *   锁住两个执行按钮、**一个请求都不发**。接口读取失败同样落这个提示
 *   （真实错误写进按钮 title 和日志，断网 ≠ 没权限，能区分开）。
 *
 * v2.1.1 - 2026-9-16（按红领巾反馈改）
 * - 去掉点按钮时的原生 window.confirm 二次确认（太打断）→ 点即执行；
 *   目标值改为先写一行日志，出问题能从日志倒推这次改了什么。
 * - @name 对齐文件名「扁鹊-1.7订单特殊审批」。
 *
 * v2.1.0 - 2026-9-16（按红领巾反馈改）
 * - 术语统一为「特殊审批 开 / 关」（原「内勤审批：已开启/已关闭」是自创叫法，已废弃）。
 * - 面板「只高亮当前启用的模式」：两个模式按钮不再按开/关上色，
 *   只有实测生效的那个是绿色 +「当前启用」角标，另一个一律弱化。
 * - 开关挪到「报表工具」**左侧**并拉开间距；独立启用（无共享工具组）时同样留间距。
 *
 * v2.0.0 - 2026-9-16
 * - 改为顶部工具区开关（与 1.4/1.5/1.6 同一位置、同一套入口逻辑），点开关展开面板。
 * - 不再跳转页面：直接发请求改两项，改完自动回读校验。
 * - 两项成对切换；补上阈值字段安全闸；@match / @grant 对齐 1.5、1.6。
 *
 * v1.0.0 - 2026-9-16
 * - 首版：右下角常驻面板 + 两个独立按钮（全部开启 / 全部关闭），② 走页面 UI 保存。
 */
